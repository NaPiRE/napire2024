...
var levelHeights = {}, changedChartHeight = false;
...
function addGridPart(path, d, context) {
  path.push(
    'M',
    pInt(d[0]), pInt(d[1]),
    'L',
...
axisProto.render = function () {
  ...
  if (typeof this.originalTickLength === "undefined")
...
// Inject the baseWidth into userAttr
tickProto.fixLabelWidths = function (userAttr, category, depth, baseWidth) {
  if (typeof userAttr == "undefined") {
    userAttr = [];
  }

  if (typeof userAttr[depth-1] == "undefined") {
    userAttr[depth-1] = {style: {width: category.leaves * baseWidth}};
  }
  else {
    if (typeof userAttr[depth-1].style == "undefined") {
      userAttr[depth-1].style = {width: category.leaves * baseWidth};
    }
    else {
      userAttr[depth-1].style.width = category.leaves * baseWidth;
...
tickProto.addGroupedLabels = function (category) {
  ...
  while (tick) {
    if (depth > 0 && !category.tick) {
      ...
      // Fix the label widths:
      // We have to fix the label widths for every item that is not in the first row of the x-axis.
      // We can do this by injecting the calculated baseWidth into the userAttr object.
      userAttr = this.fixLabelWidths(userAttr, category, depth, baseWidth);
    ...
    // Calculate the baseWidth
    if (typeof tick.leaves == "undefined" && typeof tick.label.textWidth != "undefined" && tick.label.textWidth != baseWidth) {
      baseWidth = tick.label.textWidth;
    }

    // set level size
    axis.groupSize(depth, tick.label.getBBox()[size]);

    if(typeof levelHeights[depth] == "undefined" || tick.label.getBBox()['height'] > levelHeights[depth]) {
      levelHeights[depth] = tick.label.getBBox()['height'];
    }
...
tickProto.render = function (index, old, opacity) {
  ...
  if(!changedChartHeight){
    var newChartHeight = this.axis.chart.chartHeight;
    for(var key in levelHeights){
      newChartHeight += levelHeights[key];
    }

    setTimeout(function(){
      var chartObject = HC.charts[HC.charts.length - 1];
      chartObject.setSize(chartObject.chartWidth, newChartHeight, false);
      chartObject.hasUserSize = false; // Hack to keep autoresizing active
    });
    changedChartHeight = true;
  }
  ...
  if (isFirst) {
    ...
    addGridPart(grid, gridAttrs,2);
  }


	if(horiz && axis.left < xy.x) {
			addGridPart(grid, [xy.x, xy.y, xy.x, xy.y + size],3);
	} else if(!horiz && axis.top < xy.y){
			addGridPart(grid, [xy.x, xy.y, xy.x + size, xy.y],4);
	}
  ...
  if(axis.opposite){
    var newHeight = 0;
    var xy = group.label.xy;
    if(tick.label.element.nodeName.toLowerCase()=="text" && tick.label.element.childNodes.length>1){
      newHeight = ($(tick.label.element.childNodes[1]).attr("dy") * (tick.label.element.childNodes.length-1)) - 4;
    }
  ...
  while (group = group.parent) {
    ...
    attrs = horiz ? {
      ...
      y: (size + lvlSize / 2 + baseLine - bBox.height / 2 )
    } : {
			x: size + lvlSize / 2,
		...
		if(!isNaN(attrs.x) && !isNaN(attrs.y)){
			...
			if (grid) {
				if(horiz && axis.left < maxPos.x) {
						addGridPart(grid, [maxPos.x, size, maxPos.x, size + lvlSize],5);
				} else if(!horiz && axis.top < maxPos.y){
						addGridPart(grid, [size, maxPos.y, size + lvlSize, maxPos.y],6);
				}
			}
    }
